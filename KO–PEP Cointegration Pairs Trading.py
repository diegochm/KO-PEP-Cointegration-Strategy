...
duplicates_x = x.index.duplicated().sum()
duplicates_y = y.index.duplicated().sum()

print(f"Duplicates in x (KO): {duplicates_x}")
print(f"Duplicates in y (PEP): {duplicates_y}")
...
plt.title('Z-Score of Spread KO–PEP (window=90)')
plt.show()
...
axes[1].legend(); axes[1].grid(True); axes[1].set_title('Z-score and Dynamics Thresholds')

df['cum_ret'].plot(ax=axes[2], label='Strategy KO−β·PEP (cum.)')
axes[2].legend(); axes[2].grid(True); axes[2].set_title('Cum Ret (without comissions)')
...
print("First day:", df.index[ready.argmax()] if ready.any() else "none")
print("¿ long? ", (df['long_signal']==1).any())
print("¿ short?',", (df['short_signal']==1).any())
